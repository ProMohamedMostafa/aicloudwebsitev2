...
// Map feature titles from data to i18n keys
      ...
      <div className="mx-4 md:mx-6 lg:mx-10 xl:mx-auto max-w-7xl">
        ...
        <div className="text-center mb-16 lg:mb-20 px-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-full text-sm font-medium mb-6">
            ...
            {t("cleanTech.features.badge")}
          </div>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-6">
            {t("cleanTech.features.title")}{" "}
            <span className="text-green-600">{t("cleanTech.features.titleHighlight")}</span>
          </h2>
        </div>
        ...
        <div className="space-y-12 lg:space-y-16">
          {features.map((feature, index) => {
            const i18nKey = featureKeyMap[feature.title] ?? feature.title;
            const featureLabel = t(`cleanTech.featuresData.${i18nKey}.label`);
            const featureTitle = t(`cleanTech.featuresData.${i18nKey}.title`);
            const featureDesc = t(`cleanTech.featuresData.${i18nKey}.description`);

            // In RTL, alternate order is mirrored: even→text-right/img-left, odd→text-left/img-right
            const textOrder = index % 2 === 0 ? "lg:order-1" : "lg:order-2";
            const imgOrder = index % 2 === 0 ? "lg:order-2" : "lg:order-1";
              ...
              <div key={index} className="group">
                <div className="w-full bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 group-hover:border-green-200">
                  <div className="grid w-full grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mx-auto px-6 py-8 lg:py-12">
                    {/* Text Content */}
                    <div
                      className={`flex flex-col justify-center space-y-4 lg:space-y-6 ${textOrder}`}
                    ...
                    >
                      ...
                      <h3 className="text-2xl lg:text-3xl xl:text-4xl font-bold text-gray-900 group-hover:text-green-700 transition-colors duration-300">
                        {featureTitle} {t("cleanTech.features.featureSuffix")}
                      </h3>
                    ...
                    {/* Image */}
                    <div className={`flex justify-center items-center ${imgOrder}`}>
